...
def _load_model_config(checkpoint_path):
  """Load current fields while accepting successful pre-refactor runs."""
  ...
  supported_fields = {
      field.name for field in dataclasses.fields(model.GridCellConfig)
  }
  return model.GridCellConfig(**{
      key: value
...
def plot_encoder_activity(activity, output_path):
  ...
  fig.suptitle('Grid-cell activity (SIREN encoder)')
  fig.tight_layout()
...
def reconstruct_encoder_activity(checkpoint_path, state_dict):
  """Reconstruct the continuous SIREN encoder on a dense spatial grid."""
...
def plot_matrix_parameter(value, name, output_dir):
  ...
  if value.ndim == 4:  # [theta, module, row, column]
    num_directions, num_modules = value.shape[:2]
    num_cols = min(6, num_directions)
    num_rows = math.ceil(num_directions / num_cols)
...
def plot_generated_direction_matrices(checkpoint_path, state_dict, output_dir):
  """Sample and plot matrices generated from continuous directions."""
  model_config = _load_model_config(checkpoint_path)

  grid_cell = model.GridCell(model_config)
  grid_cell.load_state_dict(state_dict)
  num_directions = 18
  theta = torch.arange(num_directions, dtype=torch.float32)
  theta *= 2 * torch.pi / num_directions
  direction = torch.stack((torch.cos(theta), torch.sin(theta)), dim=-1)
  with torch.no_grad():
    matrices = grid_cell.trans.B_mlp(direction).reshape(
        num_directions, grid_cell.trans.num_module,
        grid_cell.trans.module_size, grid_cell.trans.module_size)
  plot_matrix_parameter(
      matrices.numpy(), 'trans.B(theta)_generated', output_dir)
...
def main():
  ...
  activity = reconstruct_encoder_activity(checkpoint_path, state_dict)
  plot_encoder_activity(activity, output_dir / 'encoder_activity.png')

  plot_generated_direction_matrices(checkpoint_path, state_dict, output_dir)

  for name, tensor in state_dict.items():
    if name.startswith('encoder.'):
      continue
    if name.startswith('trans.B_mlp.'):
      continue
